...
export default function TakeawayCard({ dailySales = [], budget = 0, storeBudget = 0, onBudgetChange }) {
  ...
  const BarChart = () => {
    ...
    if (!pts || pts.length === 0) return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 80, color: '#d1d5db', fontSize: 11 }}>
        Sin datos este mes
      </div>);

    const W = 480,H = 80,barW = Math.max(2, W / pts.length - 3);
    const max = Math.max(...pts.map((p) => p.value), 1);
    const avg = analysis.dailyAvg;
    return (
      <svg viewBox={`0 0 ${W} ${H + 20}`} fill="none" style={{ width: '100%', height: 'auto' }}>
        <defs>
        ...
        {avg > 0 && (() => {
          ...
          return (
            <>
              <line x1="0" y1={avgY} x2={W} y2={avgY} stroke={PINK} strokeWidth="0.8" strokeDasharray="4 3" strokeOpacity="0.35" />
              <text x={W - 2} y={avgY - 3} textAnchor="end" style={{ fontSize: 7, fill: PINK, opacity: 0.5, fontWeight: 600, fontFamily: 'Inter Tight, sans-serif' }}>avg</text>
            </>);
        ...
        {pts.map((p, i) => {
          ...
          return (
            <g key={i}>
              ...
              {isTop && <rect x={x} y={y} width={barW} height={barH} rx="2" ry="2" fill="none" stroke={PINK} strokeWidth="0.6" strokeOpacity="0.4" />}
              {(i % 3 === 0 || isLast) &&
      ...
      <AnimatePresence>
        ...
        <motion.div
          ...
          style={{ borderTop: `1px solid ${PINK_SOFT}`, overflow: 'hidden' }}>
            ...
            { label: 'Promedio/día', value: fmt(analysis.dailyAvg), sub: analysis.dailyNeeded != null ? `meta ${fmt(analysis.dailyNeeded)}` : `${analysis.daysRemaining}d rest.`, color: '#374151', subColor: '#94a3b8' },
            ...
            {/* ── CHART ── */}
            <div style={{ padding: '12px 14px 8px', background: 'linear-gradient(180deg, #fff8fb 0%, #ffffff 100%)' }}>
